...
- [Install in AWS EKS](#install-in-aws-eks)
  ...
  - [Install in AWS EKS sing Fargate](#install-in-aws-eks-using-fargate)
...
### 6. Note down `Host:Port` information

The cluster IP can be checked by running
...
levoai-collector   ClusterIP   10.100.254.104   <none>        4317/TCP       4m8s
```
The Collector can now be reached by the Sensors running in the same VPC in AWS at port `< CLUSTER IP >:4317`. Please note this, as it will be required to configure the Sensor.
...
```bash
eksctl create cluster --name HostedSatellite--region us-west-2 --fargate 
```
...
helm upgrade --install -n levoai --create-namespace \
  --set global.levoai_config_override.onprem-api.refresh-token=$LEVOAI_AUTH_KEY \
  --set global.levoai_config_override.traces_queue.type="sqs" --set global.useRabbitmq=false \
  levoai-satellite levoai/levoai-satellite 
```
