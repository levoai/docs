...
export function SensorConfigFile() {
  return (
    <BrowserOnly fallback={<div>Loading...</div>}>
      {() => (
        <a href={window.location.protocol + '//' + window.location.host + '/artifacts/common-tasks/config.yml'} download> YAML configuration file</a>
      )}
    </BrowserOnly>
  );
}
...
export function ConfigurationValues() {
  return (
    <BrowserOnly fallback={<div>Loading...</div>}>
      {() => (
        <a href={window.location.protocol + '//' + window.location.host + '/artifacts/common-tasks/config-values.yml'} download> Configuration Values</a>
      )}
    </BrowserOnly>
  );
}
...
# that the common-tasks is installed in (e.g. staging, prod, dev).
#
...
# --------------------------------------------------------------------------------------------
# By default, the common-tasks rate-limits the overall number of request-response traces it exports
...
# Metrics Settings:
# --------------------------------------------------------------------------------------------
# Uncomment and modify to enable the printing of common-tasks metrics alongside other common-tasks logging
# messages.
...
helm upgrade levoai-common-tasks levoai/levoai-ebpf-common-tasks \
  ...
  --namespace levoai \
  --create-namespace \
  --set common-tasks.otel.grpcEndpoint=<hostname|IP:port> \
...
helm upgrade levoai-common-tasks levoai/levoai-ebpf-common-tasks \
  ...
  --namespace levoai \
  --create-namespace \
  --set common-tasks.config.default-service-name="my-test-app-k8s-c101" \
...
sudo docker run --restart unless-stopped \
  ...
  -v $PWD/config.yml:/etc/levo/common-tasks/config.yaml:ro \
  --privileged --detach \
