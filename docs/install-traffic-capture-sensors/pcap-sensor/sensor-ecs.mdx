...
export function DownloadMainTF() {
    return (
        <BrowserOnly fallback={<div>Loading...</div>}>
            {() => (
                <a href={window.location.protocol + '//' + window.location.host + '/artifacts/sensor/pcap/main.tf'} download> main.tf </a>
            )}
        </BrowserOnly>
    );
}
...
> *Enter the inputs as prompted to create a task definition in your AWS account.*


</TabItem>
<TabItem value="fargate">
