...
export function DownloadSatEBPF() {
    return (
        <BrowserOnly fallback={<div>Loading...</div>}>
            {() => (
                <a href={window.location.protocol + '//' + window.location.host + '/artifacts/satellite/for-ebpf/main.tf'} download> main.tf </a>
            )}
        </BrowserOnly>
    );
}

export function DownloadSatPCAP() {
    return (
        <BrowserOnly fallback={<div>Loading...</div>}>
            {() => (
                <a href={window.location.protocol + '//' + window.location.host + '/artifacts/satellite/for-pcap/main.tf'} download> main.tf </a>
            )}
        </BrowserOnly>
    );
}
...
<TabItem value="tf">
You can download the particular satellite according to your Packet Capture Sensor.
<br /><br />

#### For EBPF Sensor:
Download the **<DownloadSatEBPF/>** file **(make sure you've installed the EBPF sensor)**.

#### For PCAP Sensor:
Download the **<DownloadSatPCAP/>** file **(make sure you've installed the PCAP sensor)**.
