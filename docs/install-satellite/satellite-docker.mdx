...
export function DownloadShellScriptLink() {
    return (
        <BrowserOnly fallback={<div>Loading...</div>}>
            {() => (
                  <a href={window.location.protocol + '//' + window.location.host + '/artifacts/satellite/levo_satellite.sh'} download> Download</a>
            )}
        </BrowserOnly>
    );
}
...
<DownloadShellScriptLink/> the Shell Script file to your desktop.

### 2. Install Satellite

Before running a .sh script, you need to ensure it has executable permissions. Once the script has executable permissions, you can run it in the terminal.

This is a command to change file permissions and adds executable permissions to the file. This allows you to run thr script directly using './levo_satellite.sh'
...
### 3. Usage
After making the script executable, you can use the following commands:

#### a. Start the Levo Satellite

Downloads the Docker Compose file if not present.

Starts the Levo Satellite Services.
...
This command stops all Levo Satellite services.
...
This command stops and then starts the Levo Satellite services.
...
- Stops and removes existing services.
- Pulls the latest Docker images.
- Starts the services with the new configuration.
...
This command:
- Collects logs from all running Levo Satellite services.
- Creates a compressed archive of the logs.
- Outputs the location of the archive file.
