...
export function DownloadGetLogsScript() {
    return (
        <BrowserOnly fallback={<div>Loading...</div>}>
            {() => (
                <a href={window.location.protocol + '//' + window.location.host + '/artifacts/satellite/get_logs.sh'} download> download</a>
            )}
        </BrowserOnly>
    );
}
...
### Extracting logs from Satellite components

To extract logs from Satellite components, please <DownloadGetLogsScript/> script and execute following commands.
...
Executing this script will collect logs from all Satellite components and creates an archive as `/tmp/levoai_satellite_logs_%date-time%.tar.gz`.
