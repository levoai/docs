...
```bash
#!/bin/bash
cat <<EOF > /opt/levo/.levoenv
LEVOAI_AUTH_KEY="YOUR_LEVOAI_AUTH_KEY"
LEVOAI_ORG_ID="Your_LEVOAI_ORG_ID"
LEVOAI_BASE_URL="Base env url"
EOF

chmod 600 /opt/levo/.levoenv
chown ubuntu:ubuntu /opt/levo/.levoenv
